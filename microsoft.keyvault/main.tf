...
resource "azurerm_key_vault_access_policy" "sac_key_vault_policy" { # SaC Testing - Severity: Critical - set access_policy to undefined
  ...
  # Drata: Explicitly scope [azurerm_key_vault_access_policy.key_permissions] to ensure minimum necessary access. Avoid using insecure allow-all ([<AccessPattern.AZ_ALL: 'all'>, <AccessPattern.AZ_DELETE: 'delete'>, <AccessPattern.AZ_PURGE: 'purge'>]) access patterns
